...
const CONTRACT_NAME: &str = "block-merkle-airdrop";
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::WithdrawAll {stage} => try_withdraw_all(deps, info, stage),
...
pub fn try_withdraw_all(deps: DepsMut, info: MessageInfo, stage: u8) -> Result<Response, ContractError> {
    ...
    // return Err(ContractError::LogAmount { total_amount, claimed_amount });
    ...
    // STAGE_AMOUNT_CLAIMED.save(deps.storage, 1, &total_amount)?;
    STAGE_AMOUNT.save(deps.storage, 1, &Uint128::zero())?;

    Ok(Response::new()
        .add_attribute("action", "withdraw_all")
        .add_attribute("amount", balance_to_withdraw)
...
pub fn try_receive(deps: DepsMut, msg: Cw20ReceiveMsg) -> Result<Response, ContractError> {
    ...
    //Only possible in stage 1
    let mut ori_amount = STAGE_AMOUNT.load(deps.storage, 1u8)?;
    ori_amount += msg.amount;
    STAGE_AMOUNT.save(deps.storage, 1u8, &ori_amount)?;
...
pub fn execute_register_merkle_root(
    ...
    //ACHILLES ADDED For only one stage
    if stage > 1 {
        return Err(ContractError::OnlyOneStage {});
    }
    //ACHILLES END
...
pub fn execute_burn(
    ...
    // Before burn set total amount to zero
    STAGE_AMOUNT.save(deps.storage, stage, &Uint128::zero())?;
...
pub fn query_total_claimed(deps: Deps, stage: u8) -> StdResult<TotalClaimedResponse> {
    ...
    let amount = STAGE_AMOUNT.load(deps.storage, stage)?;
    let resp = TotalClaimedResponse { amount, total_claimed };
